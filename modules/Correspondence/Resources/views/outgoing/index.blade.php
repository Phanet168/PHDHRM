...
@section('title', localize('outgoing_letters', 'លិខិតចេញ'))
    ...
    <div class="body-content">
        ...
        <div class="card corr-card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <h6 class="mb-0">{{ localize('outgoing_letters', 'លិខិតចេញ') }}</h6>
                <div class="d-flex gap-2 flex-wrap align-items-center">
                    <a href="{{ route('correspondence.help', ['article' => 'outgoing']) }}" class="btn btn-sm btn-outline-info">
            ...
            <div class="card-body border-bottom py-2">
                <form method="GET" class="row g-2 align-items-end">
                    ...
                    <div class="col-lg-2 col-md-4 d-flex gap-2">
                        ...
                        <a href="{{ route('correspondence.outgoing') }}" class="btn btn-sm btn-outline-secondary">{{ localize('reset', 'កំណត់ឡើងវិញ') }}</a>
                    </div>
                </form>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            @forelse ($letters as $index => $letter)
                                ...
                                @php
                                    ...
                                    $recipientText = '';
                                    if (!empty($toNames)) {
                                        $recipientText .= 'To: ' . implode(', ', $toNames);
                                    }
                                    if (!empty($ccNames)) {
                                        $recipientText .= ($recipientText !== '' ? ' | ' : '') . 'CC: ' . implode(', ', $ccNames);
                                    }
                                @endphp
                                <tr>
                                    <td>{{ $letters->firstItem() + $index }}</td>
                                    <td>
                                        <div class="fw-semibold">{{ $letter->letter_no ?: '-' }}</div>
                                        <div class="small text-muted">{{ $letter->registry_no ?: '-' }}</div>
                                    </td>
                                    <td>
                                        ...
                                        <div class="small text-muted d-flex flex-wrap gap-1 mt-1">
                                            ...
                                            @if($letter->from_org)
                                                <span class="corr-soft-chip">{{ $letter->from_org }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td>
                                        <div>{{ optional($letter->letter_date)->format('d/m/Y') ?: '-' }}</div>
                                        <div class="small text-muted">{{ localize('sent_date', 'ថ្ងៃផ្ញើចេញ') }}: {{ optional($letter->sent_date)->format('d/m/Y') ?: '-' }}</div>
                                    </td>
                                    <td>
                                        @php
                                            $totalDist = (int) ($letter->distributions_total_count ?? 0);
                                            $doneDist = (int) ($letter->distributions_done_count ?? 0);
                                        @endphp
                                        <div>{{ optional($letter->sent_date)->format('d/m/Y') ?: '-' }}</div>
                                        <div class="small text-muted">{{ $doneDist }}/{{ $totalDist }} {{ localize('received', 'បានទទួល') }}</div>
                                    </td>
                                    ...
                                    <td>
                                        @if ($recipientText !== '')
                                            <div class="small">{{ $recipientText }}</div>
                                        @else
                                            -
                                        @endif
                                    </td>
