...
@section('title', localize('edit_report', 'Edit report'))
    ...
    <div class="body-content">
        ...
        <div class="card pharm-card">
            <div class="card-header">
                <h6 class="mb-0">{{ localize('edit_pharm_report', 'Edit pharmaceutical report') }} – {{ $report->reference_no }}</h6>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('pharmaceutical.reports.update', $report->id) }}" id="reportForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="department_id" id="departmentSelect" class="form-select" required>
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept->id }}" {{ $report->department_id == $dept->id ? 'selected' : '' }}>{{ $dept->department_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">{{ localize('report_to', 'Report to') }}</label>
                            <input type="hidden" name="parent_department_id" id="parentDeptId" value="{{ $report->parent_department_id }}">
                            <input type="text" id="parentDeptDisplay" class="form-control" readonly placeholder="--" style="background:#f8f9fa;"
                                value="{{ $report->parentDepartment?->department_name ?? '--' }}">
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="report_type" class="form-select" required>
                                @foreach(\Modules\Pharmaceutical\Entities\PharmReport::typeLabels() as $key => $label)
                                    <option value="{{ $key }}" {{ $report->report_type === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">{{ localize('period_label', 'Period label') }}</label>
                            <input type="text" name="period_label" class="form-control" value="{{ $report->period_label }}" placeholder="e.g. 2026-03, 2026-Q1">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">{{ localize('period_start', 'Period start') }} <span class="text-danger">*</span></label>
                            <input type="date" name="period_start" class="form-control" value="{{ $report->period_start?->format('Y-m-d') }}" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">{{ localize('period_end', 'Period end') }} <span class="text-danger">*</span></label>
                            <input type="date" name="period_end" class="form-control" value="{{ $report->period_end?->format('Y-m-d') }}" required>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label">{{ localize('note', 'Note') }}</label>
                            <textarea name="note" class="form-control" rows="2">{{ $report->note }}</textarea>
                        </div>
                    </div>
                        ...
                        <table class="table table-bordered table-sm" id="reportItemsTable">
                            ...
                            <tbody>
                                ...
                                <tr class="rpt-item-row">
                                    <td>
                                        <select name="items[{{ $i }}][medicine_id]" class="form-select form-select-sm" required>
                                            <option value="">--</option>
                                            @foreach($medicines as $med)
                                                <option value="{{ $med->id }}" {{ $item->medicine_id == $med->id ? 'selected' : '' }}>{{ $med->code }} – {{ $med->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td><input type="number" name="items[{{ $i }}][opening_stock]" class="form-control form-control-sm" min="0" step="0.01" value="{{ $item->opening_stock }}" required></td>
                                    <td><input type="number" name="items[{{ $i }}][received_qty]" class="form-control form-control-sm" min="0" step="0.01" value="{{ $item->received_qty }}" required></td>
                                    <td><input type="number" name="items[{{ $i }}][dispensed_qty]" class="form-control form-control-sm" min="0" step="0.01" value="{{ $item->dispensed_qty }}" required></td>
                                    <td><input type="number" name="items[{{ $i }}][damaged_qty]" class="form-control form-control-sm" min="0" step="0.01" value="{{ $item->damaged_qty ?? 0 }}"></td>
                                    <td><input type="number" name="items[{{ $i }}][expired_qty]" class="form-control form-control-sm" min="0" step="0.01" value="{{ $item->expired_qty }}"></td>
                                    <td><input type="number" name="items[{{ $i }}][adjustment_qty]" class="form-control form-control-sm" step="0.01" value="{{ $item->adjustment_qty }}"></td>
                                    <td><input type="number" name="items[{{ $i }}][closing_stock]" class="form-control form-control-sm" min="0" step="0.01" value="{{ $item->closing_stock }}" required></td>
                                    <td><input type="text" name="items[{{ $i }}][note]" class="form-control form-control-sm" value="{{ $item->note }}"></td>
                    ...
                    <div>
                        ...
                        <a href="{{ route('pharmaceutical.reports.show', $report->id) }}" class="btn btn-secondary">{{ localize('cancel', 'Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
