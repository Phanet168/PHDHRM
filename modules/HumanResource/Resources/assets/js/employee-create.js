...
$("#add_doc_row").click(function (e) {
    e.preventDefault();
    if (count > 5) {
        alert(kh("Maximum 5 documents allowed", "អាចបន្ថែមឯកសារបានអតិបរមា 5 ឯកសារ"));
        return false;
    }
...
function updateCivilServantPhasePreview() {
    ...
    var probationLabel = $phase.data("probationLabel") || kh("Probation civil servant", "មន្ត្រីចុះកម្មសិក្សា");
    var fullRightLabel = $phase.data("fullRightLabel") || kh("Full-right civil servant", "មន្ត្រីពេញសិទ្ធ");
...
function familyMemberRowTemplate(index) {
    ...
    var deleteLabel = t("delete", kh("Delete", "លុប"));
    var yesLabel = t("yes", kh("Yes", "បាទ/ចាស"));
    var noLabel = t("no", kh("No", "ទេ"));
...
function rowTemplate(repeater, index) {
    if (repeater === "pay_grade_histories") {
        return "<tr>" +
            ...
            '<td><select name="pay_grade_histories[' + index + '][status]" class="form-select"><option value="active">' + kh("Active", "សកម្ម") + '</option><option value="inactive">' + kh("Inactive", "អសកម្ម") + "</option></select></td>" +
...
function updateAgeDisplay() {
    ...
    ageInput.val((years >= 0 ? years : 0) + (isKhmerLocaleUi() ? " ឆ្នាំ" : " years"));
}
