...
/**
 * Prevent accidental navigation/submission to invalid URLs like `undefined`,
 * which currently causes requests such as `/hr/undefined` (404).
 */
(function () {
    "use strict";

    function isInvalidRouteValue(value) {
        ...
        try {
            ...
            return /\/undefined(?:\/|\?|#|$)/i.test(parsed.pathname);
        } catch (err) {
            return false;
        }
    }
    ...
    $(document).on('click', 'a[href], area[href]', function (event) {
        ...
        if (isInvalidRouteValue(href)) {
            event.preventDefault();
            event.stopImmediatePropagation();
            console.warn('Blocked invalid link navigation:', href, this);
        }
    });
    ...
    $(document).on('submit', 'form', function (event) {
        ...
        if (isInvalidRouteValue(action)) {
            event.preventDefault();
            event.stopImmediatePropagation();
            console.warn('Blocked invalid form submission:', action, this);
        }
    });
})();
