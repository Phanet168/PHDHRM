...
$streetToken = '${current_street_no_inline}';
$streetPos = mb_strpos($xml, $streetToken);
$villagePos = $streetPos !== false ? mb_strpos($xml, '<w:t>នៅភូមិ</w:t>', $streetPos) : false;
if ($streetPos === false || $villagePos === false) {
    $zip->close();
    fwrite(STDERR, "Street/village markers not found\n");
    exit(1);
}
