...
class AttendanceWorkflowNotificationService
{
    ...
    public function notifySubmitted(AttendanceAdjustment $adjustment): void
    {
        ...
        $firstStep = $this->currentWorkflowStep($adjustment);

        if ($requester = $this->requesterUser($adjustment)) {
            $requester->notify(new AttendanceWorkflowNotification(
                $adjustment,
                'submitted',
                localize('attendance_adjustment_submitted', '?????????????????????????????????????????????????????????????????????????????????'),
                localize('attendance_adjustment_submitted_message', '????????????????????????????????????????????????????????????????????????????????????????????????????????? ???????????????????????????????????????????????????????????????????????????'),
                route('attendance-adjustments.index'),
                [
                    'audience_key' => 'requester',
                    'audience_label' => localize('notification_audience_requester', '???????????'),
                ]
            ));
        }
        ...
        foreach ($this->currentStepRecipients($adjustment) as $recipient) {
            ...
            $recipient->notify(new AttendanceWorkflowNotification(
                $adjustment,
                'pending_review',
                localize('attendance_adjustment_waiting_for_you', '??????????????????????????????????????????????????????????????????????????????????????????'),
                localize('attendance_adjustment_waiting_for_you_message', '???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? ??????????????????????????????????????????'),
                route('attendance-adjustments.index'),
                [
                    'audience_key' => 'reviewer',
                    'audience_label' => localize('notification_audience_reviewer', '???????????/??????'),
                    'step_name' => (string) ($firstStep['step_name'] ?? ''),
                    'step_order' => (int) ($firstStep['step_order'] ?? 0),
                    'action_type' => (string) ($firstStep['action_type'] ?? ''),
                    'is_final_approval' => (bool) ($firstStep['is_final_approval'] ?? false),
                ]
            ));
        }
    }

    protected function currentWorkflowStep(AttendanceAdjustment $adjustment): ?array
    {
        ...
        return is_array($firstStep) ? $firstStep : null;
    }
