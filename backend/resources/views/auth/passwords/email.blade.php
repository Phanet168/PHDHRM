                    ...
                    <form method="POST" action="{{ route('password.telegram') }}" class="register-form">
                        ...
                        <div class="form-group">
                            <input type="text" name="login" class="form-control @error('login') is-invalid @enderror"
                                placeholder="{{ localize('email_or_username', 'Email or Username') }}" value="{{ old('login') }}">
                        </div>
                        @error('login')
                        ...
                        <button type="submit" class="btn btn-outline-success btn-block mt-3 w-100">
                            {{ localize('send_reset_link_via_telegram', 'Send Reset Link via Telegram') }}
                        </button>
                        <p class="text-muted text-center mt-2 mb-0 small">
                            {{ localize('telegram_reset_note', 'This works only if your account is already linked with Telegram.') }}
                        </p>
                    </form>
                </div>
            </div>
